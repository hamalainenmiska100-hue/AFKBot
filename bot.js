...
function save() { fs.writeFileSync(STORE, JSON.stringify(users, null, 2)); }
function saveActiveSessions() { fs.writeFileSync(REJOIN_STORE, JSON.stringify(activeSessionsStore, null, 2)); }
function saveConfig() { fs.writeFileSync(CONFIG_STORE, JSON.stringify(botConfig, null, 2)); }
...
function adminPanelComponents() {
  ...
  if (sessions.size > 0) {
    const options = []; let count = 0;
    for (const [uid, session] of sessions) {
      if (count >= 25) break; 
      options.push({ label: `User: ${uid}`, description: `Online`, value: uid }); count++;
    }
...
function getAdminStatsEmbed() {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: "📊 System", value: `**RAM:** ${ramMB} MB`, inline: true },
      { name: "🤖 Bots", value: `**Active:** ${sessions.size}`, inline: true },
...
setInterval(() => {
    for (const [uid, s] of sessions) {
        ...
        const gravity = 0.08;
        if (!s.onGround) s.velocity.y -= gravity;
        ...
        try {
            s.client.write("player_auth_input", {
                ...
                move_vector: { x: 0, z: 0 }, head_yaw: s.yaw, input_data: 0n,
...
function stopSession(uid) {
  const s = sessions.get(uid);
  if (activeSessionsStore[uid]) { delete activeSessionsStore[uid]; saveActiveSessions(); }
  if (!s) return false;
...
function handleAutoReconnect(uid) {
    ...
    s.reconnectTimer = setTimeout(() => {
        if (sessions.has(uid) && !sessions.get(uid).manualStop) startSession(uid, null, true);
    }, 60000);
}
...
async function startSession(uid, interaction, isReconnect = false) {
  const u = getUser(uid);
  if (!activeSessionsStore[uid]) { activeSessionsStore[uid] = true; saveActiveSessions(); }
  if (!u.server) { 
      if (!isReconnect) await interaction.editReply("⚠ Configure server settings first."); 
      delete activeSessionsStore[uid]; saveActiveSessions(); return; 
  }
  ...
  try {
      if (!isReconnect) {
          connectionEmbed.setDescription(`${shouldSkipPing ? "⏩ Skipping Ping" : "🔍 Pinging server..."}\n🌐 \`${ip}:${port}\``);
          await interaction.editReply({ embeds: [connectionEmbed], components: [] });
      }
      ...
      if (!isReconnect) {
          connectionEmbed.setDescription(`✅ **Connecting...**\n🌐 \`${ip}:${port}\``);
          await interaction.editReply({ embeds: [connectionEmbed] });
      }
  } catch (err) {
      if (isReconnect) return handleAutoReconnect(uid);
      connectionEmbed.setDescription(`❌ **Connection Failed**\nServer offline or unreachable.`).setThumbnail(null);
      await interaction.editReply({ embeds: [connectionEmbed] });
      delete activeSessionsStore[uid]; saveActiveSessions(); return;
  }
  ...
  const opts = { 
      ...
      viewDistance: 4, profilesFolder: getUserAuthDir(uid), username: uid, offline: false
  };
  ...
  const currentSession = { 
      ...
      position: null, velocity: (Vec3) ? new Vec3(0, 0, 0) : null,
  ...
  mc.on("spawn", () => {
    ...
    if (!isReconnect) {
        connectionEmbed.setDescription(`🟢 **Online** on \`${ip}:${port}\``).setThumbnail(null);
        interaction.editReply({ embeds: [connectionEmbed] });
    }
    performAntiAfk();
  });
  ...
  mc.on("close", () => { if (!currentSession.manualStop) handleAutoReconnect(uid); });
}
